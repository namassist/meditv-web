...
export function detectBrowserCapabilities(
  ...
  return {
    audio: {
      label: "Audio",
      status:
        input.hasSpeechSynthesis || input.hasAudioContext
          ? "ready-to-request"
...
export function MeditvHeader({
  ...
  return (
    <header
      ...
      style={{
        background: `linear-gradient(135deg, var(--meditv-header-from), var(--meditv-header-to))`,
      }}
    >
...
it("submits the six-digit code and then navigates to /screen", async () => {
  ...
  render(
    <PairingScreen
      onSubmit={onSubmit}
      onUnlock={vi.fn()}
      capabilityState={{ audio: { label: "Audio", status: "ready" } }}
    />,
  );
...
export function BootstrapRoute({
  ...
  restore: () => Promise<{
    status: "restored" | "missing" | "invalid-session" | "retryable-error";
  }>;
